...
/* Private define ------------------------------------------------------------*/
#define OVERLOAD                10
#define CONVERSION_OVERLOAD_1   100//40
#define CONVERSION_OVERLOAD_2   125//65
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
T_CON_TYPE cmd_ready = conIdle;
extern uint32_t Meter_value;
extern uint32_t Meter_value_pre;
extern uint16_t Delta_time;
extern volatile BOOL Zw_connect_Flag;
extern volatile BOOL Flow_status;
...
uint16_t Couter;
  ...
  */
   ...
   {
      Couter = TIM1_GetCounter();
      TIM1_SetCounter(0);
      Meter_value += Couter*40/3;
      Delta_time++;
      if((Zw_connect_Flag == TRUE)&&(Flow_status == TRUE))
      {
        uart_cmd.zw_uartcommandreport.length = 16;
        uart_cmd.zw_uartcommandreport.cmd = COMMAND_METER;
        if(((Delta_time-1)%60)&&(Couter!=0))
        {
          uart_cmd.zw_uartcommandreport.type = ZW_METER_REPORT;
        }
        else
        {
          uart_cmd.zw_uartcommandreport.type = ZW_METER_SET;
        }
